...
import { CancelablePromise, ChatObjectService, RoomService } from '../apis';
...
const props = defineProps<{
  id: string | number;
  portrait?: string;
  type: 'chatObjectId' | 'sessionUnitId';
  title?: string;
}>();
...
// const { portrait, tryChangePortrait } = useDestination(props.chatObjectId!);
...
const service = (file: any): CancelablePromise<ChatObjectDto> => {
  switch (props.type) {
    case 'chatObjectId':
      return ChatObjectService.uploadPortrait({
        id: Number(props.id),
        file,
    ...
    case 'sessionUnitId':
      return RoomService.uploadPortrait({
        sessionUnitId: props.id.toString(),
        file,
        onUploadProgress(progressEvent) {
          console.log('uploadFile onUploadProgress', progressEvent);
        },
    ...
    default:
      throw new Error(`props.type:${props.type}`);
  }
};
const onSubmit = () => {
